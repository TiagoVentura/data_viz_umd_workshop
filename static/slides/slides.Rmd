...
- Show a complete Data Visualization workflow. 

--

- How to prepare your data before plotting.
--

- Show you how to go from your models to nice graphs. 
...
- Theoretical: To map a variable to a plot, you need this variables to be defined as an single column. 

- Practical:  all the tidyverse packages (dplyr, purrr, tidyr, ggplot...) work with tidy data. 
...
#### Pooling data from the last US Presidential Election (The Economist).
...
- **The Mapping Step:** Tell ggplot **what** variables -> visuals you want to see. (aesthethics)
...
# Toy Example: Polls Over Time (Ugly but with the basics)
...
- From this section forward, we will use the same logic to go from your model's objects to plots. 
...
results %>% 
  ...
  coord_flip() +
  theme_minimal() +
  labs(title="Example Point Estimates")
```
]
...
- `Purr` will iterate through the list of datasets, and run the same models for each dataset. 
