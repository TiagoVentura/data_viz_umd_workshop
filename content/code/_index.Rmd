...
The last function sets your R environment to use your newly created theme in all your plots. Lets see this in our old graph:
...
Anothe option is to use one of the pre-set themes in R
...
Not a nice plot, but just an example. 
```{r}
results %>% 
  mutate(lb=estimate - 1.96*std.error, 
         up=estimate+ 1.96*std.error) %>%
  ggplot(data=., aes(y=estimate, ymin=lb, ymax=up, x=term)) +
  geom_pointrange() +
  coord_flip()
```
...
ggplot(to_plot, aes(x=fct_rev(state),y=estimate, ymin=lb, ymax=ub)) +
  geom_pointrange(shape=21, fill="tomato2", color="black", alpha=.8) +
